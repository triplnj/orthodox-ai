...
export function PrayerPlanForm() {
  const [experienceLevel, setExperienceLevel] = useState("beginner");
  const [availableTime, setAvailableTime] = useState("10 minutes per day");
  const [focus, setFocus] = useState("consistency and attention");
  ...
  async function createPrayerPlan() {
    setIsLoading(true);
    setError(null);
    setPlan(null);
    ...
    try {
      const response = await fetch("/api/pro/prayer-plan", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setError(data.error ?? "Could not create prayer plan.");
      ...
      setError("Network error while creating prayer plan.");
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div>
        ...
        <p className="mt-3 text-sm leading-6 text-gray-600">
          Create a realistic Orthodox Christian prayer rhythm based on your
          experience level, available time, and current focus.
        </p>
      </div>
      ...
      <div className="mt-6 space-y-4">
        <div>
          ...
          >
            ...
            <option value="returning to prayer">Returning to prayer</option>
            <option value="regular but inconsistent">
              Regular but inconsistent
            </option>
            <option value="already has a prayer rule">
              Already has a prayer rule
            </option>
          </select>
        </div>
        ...
        >
          {isLoading ? "Creating plan..." : "Create prayer plan"}
        </button>
      </div>
